...
.. list-table::
   ...
   * - ``modules/``
     - MicroPython C/C++ 绑定（``sensor``、``image``、``display``、``imageio``、 ``espdl``，以及随 target 启用的 ``h264`` 和 ``rtsp``）。
   * - ``components/``
...
- MicroPython 移植侧：``overlay/micropython/ports/esp32/boards/<BOARD>/`` （IDF 目标、sdkconfig、分区表、USB 字符串）。
- ESP-VISION 侧：``boards/<BOARD>/``（``boardconfig.h``、``imlib_config.h``、 ``manifest.py``，以及可选的 ``camera.c`` / ``display.c`` / ``sdcard.c``）。
...
.. list-table::
   ...
   * - ``overlay/micropython/ports/esp32/boards/<BOARD>/``
     - ``mpconfigboard.cmake`` 定义 IDF target 和 sdkconfig 链， ``mpconfigboard.h`` 定义 MicroPython 功能开关，此外还包含分区表和开发板元数据。
   * - ``boards/<BOARD>/``
     - 引脚与运行时配置、``imlib_config.h``、冻结清单，以及可选的相机、显示和 SD 卡后端。
...
ESP-VISION 自有代码以 Apache License 2.0 发布。引入的代码保持其源文件、SPDX 标识或 上游许可证声明的原始许可证。
...
各文件的许可证义务以及与上游项目的关系见 :doc:`../project-relationship/index`\ 。
