...
修改配置前，应先确定变更是面向所有 ESP-VISION 固件，还是仅面向某块开发板。修改根目录 ``micropython.cmake`` 会影响所有满足对应芯片条件的开发板；修改 ``boards/<BOARD>/``（包括其中的 MicroPython 移植文件 ``boards/<BOARD>/port/``）仅影响对应开发板。对于产品变体，应按照 :doc:`add-board` 创建独立板级包，而不是直接修改共享的开发板配置。
