...
static esp_err_t esp_vision_camera_start_xclk(void)
{
    ...
    esp_err_t ret = esp_cam_sensor_xclk_allocate(ESP_CAM_SENSOR_XCLK_ESP_CLOCK_ROUTER, &s_camera.xclk_handle);
    ESP_RETURN_ON_ERROR(ret, TAG, "failed to allocate camera xclk");

    ret = esp_cam_sensor_xclk_start(s_camera.xclk_handle, &xclk_config);
    if (ret != ESP_OK) {
        esp_cam_sensor_xclk_free(s_camera.xclk_handle);
        s_camera.xclk_handle = NULL;
        ESP_LOGE(TAG, "failed to start camera xclk: %s", esp_err_to_name(ret));
        return ret;
    }
    return ESP_OK;
#else
...
{
    if (s_camera.xclk_handle != NULL) {
        esp_cam_sensor_xclk_stop(s_camera.xclk_handle);
        esp_cam_sensor_xclk_free(s_camera.xclk_handle);
        s_camera.xclk_handle = NULL;
    }
}
...
static esp_err_t esp_vision_camera_init_ppa(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_cache_get_alignment(MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA, &cache_align),
                        TAG,
                        "failed to get DMA cache alignment");
    if (cache_align == 0) {
        cache_align = ESP_VISION_DMA_ALIGNMENT;
    }
...
static esp_err_t esp_vision_camera_init_buffers(void)
{
    ...
    if (req.count < ESP_VISION_CAMERA_BUFFER_COUNT) {
        ESP_LOGE(TAG, "camera returned insufficient buffers");
        return ESP_ERR_NO_MEM;
    }

    for (uint32_t i = 0; i < ESP_VISION_CAMERA_BUFFER_COUNT; i++) {
...
esp_err_t esp_vision_camera_init(void)
{
    ...
    ret = esp_vision_camera_start_xclk();
    if (ret != ESP_OK) {
        goto fail;
    }
...
bool esp_vision_camera_is_ready(void)
{
    return s_camera.initialized &&
           ...
           (s_camera.ppa_handle != NULL) &&
           (s_camera.ppa_out_buf != NULL) &&
           (s_camera.input_pixfmt == V4L2_PIX_FMT_RGB565) &&
           (s_camera.width != 0) &&
           (s_camera.height != 0);
}
