...
Notes:
- `BOARD` must exist as `boards/<BOARD>/` with a `boards/<BOARD>/port/` subdirectory (the MicroPython-port files projected onto the esp32 port at build time). Boards: `ESP32_P4X_EYE`, `ESP32_P4X_FUNCTION_EV_BOARD`, `ESP32_S3_EYE`, `ESP32_S31_KORVO`; `TEMPLATE` is for new-board bring-up.
