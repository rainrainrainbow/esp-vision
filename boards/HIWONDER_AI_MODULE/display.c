...
{
    if (backlight > 100U) backlight = 100U;
...
esp_err_t esp_vision_board_display_init_panel(uint32_t width, uint32_t height,
                                              esp_lcd_panel_io_handle_t *io_handle,
                                              esp_lcd_panel_handle_t *panel_handle)
{
    if ((width != 320) || (height != 240) || !io_handle || !panel_handle)
    ...
    *panel_handle = NULL;
    ...
    if (ret != ESP_OK) { spi_bus_free(spi_host); return ret; }
    ...
    (*panel_handle)->draw_bitmap = esp_vision_hiwonder_draw_bitmap;
...
void esp_vision_board_display_deinit_panel(esp_lcd_panel_io_handle_t io_handle,
                                           esp_lcd_panel_handle_t panel_handle)
{
    if (panel_handle) { esp_lcd_panel_disp_on_off(panel_handle, false); esp_lcd_panel_del(panel_handle); }
