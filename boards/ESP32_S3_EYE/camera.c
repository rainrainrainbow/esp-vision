 ...
 *
 * ESP32-S3-EYE GC2145 camera backend using esp_video_init / V4L2.
 * Ported from ESP32_S31_KORVO pattern.
 */
...
static esp_err_t esp_vision_camera_read_sensor_id(uint8_t addr, uint16_t high_reg,
                                                   uint16_t low_reg, uint32_t *id)
{
    uint8_t high = 0;
    uint8_t low = 0;
...
static esp_err_t esp_vision_camera_video_init(void)
{
    ...
    /* Probe GC2145 manually for diagnostics */
    ret = esp_vision_camera_probe_sensor();
    if (ret != ESP_OK) {
        esp_vision_debug_printf("[esp-vision] camera init: manual probe failed (auto-detect may still work)\r\n");
    }
...
static esp_err_t esp_vision_camera_configure_input_format(uint32_t width, uint32_t height, uint32_t pixfmt)
{
    ...
    if (!esp_vision_camera_is_rgb565_input(format.fmt.pix.pixelformat)) {
    ...
    s_camera.input_stride = format.fmt.pix.width * sizeof(uint16_t);
    return ESP_OK;
}
...
esp_err_t esp_vision_camera_init(void)
{
    ...
    esp_vision_debug_printf("[esp-vision] camera init failed: stage=%s ret=%d\r\n", stage, (int)ret);
    return ret;
}
...
bool esp_vision_camera_is_ready(void)
{
    return s_camera.initialized &&
           s_camera.streaming &&
           esp_vision_camera_is_rgb565_input(s_camera.input_pixfmt) &&
           (s_camera.width != 0) &&
           (s_camera.height != 0);
}
...
esp_err_t esp_vision_camera_capture(uint8_t *pixels, size_t pixels_size)
{
    ...
    /* Direct copy from DVP buffer — GC2145 outputs RGB565_BE natively.
     * When raw_input == output, this is a straight memcpy;
     * future: add PPA scaling pipeline if needed. */
    {
        size_t copy_size = expected_size < input_size ? expected_size : input_size;
        memcpy(pixels, s_camera.buffers[buf.index].ptr, copy_size);
    }
